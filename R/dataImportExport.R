iNZDataImportExport <- setRefClass("iNZDataImportExport",
                                fields = list(
                                    dataSet = "data.frame",
                                    error = "list"
                                    ),
                                methods = list(
                                    ...
                                    importData = function(theFile, ext) {
                                        ...
                                        if(length(ext) == 0)
                                            ...
                                            setError("Chosen file is different than the selected file type", "Error")
                                            ...
                                            if (inherits(out, "try-error")) {
                                                ...
                                                dataSet <<- out
                                                ## if data set is less that 200000,
                                                ## enable dataView, disable list View, updateData()
                                                ## else
                                                ## disable both views, updateList()
                                                ## reset Plot settings
                                                ## clear the Drag fields
                                                ## close import window
                                            }
                                        } else if (ext %in% c("xls", "xlsx")) {
                                            if (ext == "xls") {
                                                channel <-  if (exists("odbcConnectExcel"))
                                                    try(RODBC::odbcConnectExcel(theFile, readOnly = TRUE, readOnlyOptimize=TRUE))
                                                else
                                                    NULL
                                                excelString <- "Excel"
                                            } else if (ext == "xlsx") {
                                                channel <- if (exists("odbcConnectExcel2007"))
                                                    try(RODBC::odbcConnectExcel2007(theFile, readOnly = TRUE, readOnlyOptimize=TRUE))
                                                else
                                                    NULL
