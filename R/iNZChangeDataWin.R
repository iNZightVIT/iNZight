...
iNZFilterWin <- setRefClass(
    ...
    methods = list(
        initialize = function(gui = NULL) {
            ...
            lbl <- glabel(paste(tr("data_remove_rows"), " :"))
            row_nums <<- gedit()
            ...
            lbl <- glabel(paste(tr("data_sample_size"), " :"))
            rand_size <<- gedit()
            addHandlerKeystroke(
                rand_size,
                function(h, ...) {
            ...
            lbl <- glabel(paste(tr("data_sample_number"), " :"))
        ...
        handle_filter = function(h, ...) {
            ...
            attr(newdata, "code") <<- gsub(
                ".dataset",
                if (iNZightTools::is_survey(.dataset)) {
        ...
        filter_value = function() {
            if (vartype == "cat" && length(cat_levels$get_index()) == 0) {
                return()
            }
            if (vartype == "num" && svalue(num_cond) == "") {
                return()
            }
...
iNZAggregateWin <- setRefClass(
    ...
    methods = list(
        initialize = function(gui) {
            ...
            g_aggvars <- gframe(paste("1. ", tr("data_ag_var1")),
                container = g_var
            )
            ...
            ### +++ Summary variables
            g_smryvars <- gframe(paste("2. ", tr("data_ag_var2")),
                container = g_var
            )
            ...
            ########################## Window buttons
            adv_chk <<- gcheckbox(tr("data_adv_mode"),
        ...
        update_preview = function() {
            if (length(aggvars$get_items() == 1) && aggvars$get_items()[1] == "") {
                return()
            }
            if (length(svalue(smryvars)) == 0) {
                return()
            }
        ...
        get_summaries = function() {
            ...
            do.call(
                rbind,
                sapply(
                    seq_along(1L:nrow(smry_tbl)),
                    function(i) {
                        w <- smry_tbl[i, 1L]
                        if (!methods::is(w, "GCheckbox") || !svalue(w)) {
                            return(NULL)
                        }
                        text <- w$get_items()
                        if (text == "Custom") {
...
iNZReorderVarsWin <- setRefClass(
    ...
    methods = list(
        initialize = function(gui) {
            ...
            addSpring(g_ctrls)
            glabel(paste(tr("data_alphabetical"), ": "), container = g_ctrls)
...
iNZReshapeWin <- setRefClass(
    ...
    methods = list(
        initialize = function(gui = NULL) {
            ...
            format_string <- glabel(tr("data_reshape")) 
            ...
            col_string <- glabel(tr("data_select_cols"), container = group1)
            ...
            checkbox <- gcheckbox(
                text = "Click to select multiple columns",
                container = group1,
                handler = function(h, ...) {
            ...
            key <<- "key"
            key_string <- glabel(tr("data_name_cols"),
                container = group1
            )
            ...
            value <<- "value"
            value_string <- glabel(tr("data_name_cols"),
                container = group1
            )
            ...
            col1 <<- ""
            label1 <- glabel(tr("data_cols_spread"),
                container = group2
            )
            col1box <- gcombobox(
                items = c("", names(GUI$getActiveData(lazy = TRUE))),
                container = group2,
                handler = function(h, ...) {
                    col1 <<- svalue(col1box)
                    if (col1 != "" & col2 != "") {
            ...
            col2 <<- ""
            label2 <- glabel(tr("data_cols_vals"),
                container = group2
            )
            col2box <- gcombobox(
                items = c("", names(GUI$getActiveData(lazy = TRUE))),
            ...
            string1 <- glabel(tr("data_original"))
            originview <- gtable(
...
iNZSeparateWin <- setRefClass(
    ...
    methods = list(
        initialize = function(gui = NULL) {
            ...
            format_string <- glabel(paste(tr("data_separate_vars"), " :"))
            ...
            col_string <- glabel(paste(tr("data_separate_cols"), " :"))
            var1 <<- gcombobox(c(" ", names(GUI$getActiveData(lazy = TRUE))),
                handler = function(h, ...) {
                    col <<- svalue(var1)
                    updateView()
...
iNZUniteWin <- setRefClass(
    ...
    methods = list(
        initialize = function(gui = NULL, cat_only = FALSE) {
            ...
            col_string <- glabel(tr("data_cols_unite"))
            ...
            sep <<- "_"
            lbl <- glabel(tr("data_val_separate"))
            font(lbl) <- list(weight = "bold")
            add(g_info, lbl, anchor = c(-1, 0), fill = TRUE)
            ...
            remove_empty <<- TRUE
            remove_empty_cb <- gcheckbox(tr("data_remove_combs"),
                handler = function(h, ...) {
            ...
            keep_na <<- TRUE
            keep_na_cb <- gcheckbox(tr("data_missing_vals"),
                checked = TRUE,
                handler = function(h, ...) {
...
iNZJoinWin <- setRefClass(
    ...
    methods = list(
        initialize = function(gui = NULL) {
            ...
            string1 <- glabel(tr("data_preview_original"))
            ...
            left_name_box <- gvbox()
            name_string <- glabel(tr("data_dup_cols"),
                container = left_name_box,
                anchor = c(-1, 0)
            )
            left_name <<- "Orig"
            left_name_string <- gedit(tr("data_or"), container = left_name_box)
            addHandlerKeystroke(
                left_name_string,
                function(h, ...) {
            ...
            string3 <- glabel(tr("data_preview_second"))
            ...
            data_name <<- glabel(tr("data_test"))
            right_name_box <- gvbox()
            name_string <- glabel(tr("data_dup_new"),
                            ...
                            if (length(data_set_names)) {
                                data_name <<- gcombobox(data_set_names,
                                    ...
                                    handler = function(h, ...) {
                                        ...
                                        i <- sapply(
                                            GUI$dataNameWidget$nameLabel$get_items(),
                                ...
                                data_name <<- glabel(tr("data_no_dataset"))
                            }
                        },
                        {
            ...
            coltbl[1, 1:4] <<- glabel(tr("data_cols_match"))

            add_body(middle)

            ## Bottom box
            bottom <- gvbox()
            preview_string2 <- glabel(tr("data_preview"), container = bottom, anchor = c(-1, 0))
            joinview <<- gpagedtable(data.frame("", stringsAsFactors = TRUE), container = bottom)
            size(joinview$table) <<- c(-1, 150)

            add_body(bottom)

            join_types_button <- gbutton(tr("data_join"),
